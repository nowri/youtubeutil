...
// constructor:
	...
	p.initPlayer = function (embedId, videoId, videoWidth, videoHeight, onReady, playerVars) {
		...
		switch (FallbackPlayer.stateLoadPlayerAPICode) {
			...
			case FallbackPlayer.STATE_LOAD_PLAYER_API_CODE_NONE:
				...
				window.onYouTubeIframeAPIReady = function() {
					...
					FallbackPlayer.runAcyncArray(
						FallbackPlayer._cueList, function(func){
							func();
						}
					);
				};
			...
			case FallbackPlayer.STATE_LOAD_PLAYER_API_CODE_PROGRESS:
				FallbackPlayer._cueList = FallbackPlayer._cueList || [];
				FallbackPlayer._cueList.push(this._onYouTubeIframeAPIReady.bind(this));
				break;

			case FallbackPlayer.STATE_LOAD_PLAYER_API_CODE_COMPLETE:
...
// <--- PlayerManager JS版へフォールバック 対応不要の場合削除
...
(function() {
	"use strict";
	if(!!window.postMessage && !swfobject.hasFlashPlayerVersion("8")) {
		return;
	}
	...
	p.initPlayer = function (embedId, videoId, videoWidth, videoHeight, onReady, playerVars) {
		...
		FallbackPlayer._cueList = FallbackPlayer._cueList || {};
		FallbackPlayer._cueList[embedId] = onReady;

		window.onYouTubePlayerReady = function(id){

			if(FallbackPlayer && FallbackPlayer._cueList && FallbackPlayer._cueList[id]){
				if(typeof FallbackPlayer._cueList[id] == "function"){
					var func = FallbackPlayer._cueList[id];
					delete FallbackPlayer._cueList[id];
					func(id, document.getElementById(id), false);
					func = null;
				}
			}
		};
